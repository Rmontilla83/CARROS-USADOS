...
export function vehicleExpiringEmail(
  name: string,
  brand: string,
  model: string,
  year: number,
  ...
  return {
    subject: `Tu publicación de ${vehicleTitle} vence en ${daysLeft} días`,
    html: `<!DOCTYPE html>
<html>
      ...
      <table width="600" cellpadding="0" cellspacing="0" style="background-color:#ffffff;border-radius:12px;overflow:hidden;">
        ...
        <tr>
          <td style="padding:40px;">
            <h2 style="margin:0 0 16px;color:#F39C12;font-size:22px;">Tu publicación vence pronto</h2>
            <p style="margin:0 0 16px;color:#4a4a4a;font-size:15px;line-height:1.6;">
              Hola ${name}, la publicación de tu <strong>${vehicleTitle}</strong> vence en
              <strong>${daysLeft} día${daysLeft !== 1 ? "s" : ""}</strong>.
            </p>
            <p style="margin:0 0 24px;color:#4a4a4a;font-size:15px;line-height:1.6;">
              Si aún no has vendido tu vehículo, puedes renovar la publicación para mantenerla activa
              y seguir recibiendo visitas.
            </p>
            <table cellpadding="0" cellspacing="0" style="margin:0 auto;">
              <tr><td style="background-color:#F39C12;border-radius:8px;">
